...
| 发送侧全局背压 | Missing → 0032 | pending 超阈 429 |
| 压测场景 | Partial | `load_test/scenarios/send_message.py`（仍依赖脆弱 psql，见 0031） |
...
# 服务已启动后
cd load_test
python run.py --scenario send_message --concurrency 50 --duration 30

# 快速
python run.py --scenario send_message --quick
```
...
| HTTP 5xx | 应接近 0（无背压时 pending 涨也可能仍 200） |
| Postgres 连接 / 锁等待 | 同会话写高峰时可能升高 |

## 6. 通过标准

- 压测结束错误率可接受（学习环境：无大面积 5xx）  
- 消息最终可 sync / 在线可见（consumer 未暂停）  
- 记录一份基线到 `load_test/baselines/`（0031）  
