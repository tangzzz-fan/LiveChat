...
> **现状 vs 商用**：[`仓库现状与商用品差距.md`](./仓库现状与商用品差距.md)  
> Go 弱基础复盘：[`Go后端技术栈与学习导读.md`](./Go后端技术栈与学习导读.md) · 源码导读：[`code-walkthrough/`](./code-walkthrough/)（messages / outbox）  
> API：[`API参考.md`](./API参考.md) · iOS 多端：[`iOS多端接入评估与实现.md`](./iOS多端接入评估与实现.md)  
> 高负载验证计划：[`高负载IM验证计划.md`](./高负载IM验证计划.md)（issues `0029`–`0034` complete） · **方法论**：[`验证链路设计方法论.md`](./验证链路设计方法论.md) · 实践手册：[`load-practice/`](./load-practice/) · iOS 抗压：[`ios-high-load-client.md`](./ios-high-load-client.md) · iOS 重写决策：[`ios-client-rewrite.md`](./ios-client-rewrite.md)  
...
| 高负载验证 | 实践手册、压测硬化、发送背压、iOS 抗压、protobuf 投递验证 | 0029–0034 | complete（tag `v0.3.0-high-load-validation`） |
| iOS 正确性 | OTP 登录 → 本地优先 → sync → WS（媒体/推送后置） | 0022–0028 | **superseded** → [0035](../issues/0035-ios-client-rewrite.md) 从零重写 |
| iOS 重写 | Spec/决策 → SPM 脚手架（HITL Xcode）→ 功能切片另开 | 0035–0037 | 0036 complete；0037 open |
...
| 11 | Outbox 积压背压 | 可观测 pending/lag；**pending 超阈时 send 返回 429 + `Retry-After`** | `backpressure`；[chaos 02](./chaos/02-outbox-backpressure.md)；[问题 15](./engineering-problems/15-high-concurrency-failure-modes.md) |
...
本仓的「容量验证」= **在本地用可控流量实证放大因子与降级语义**，不是宣称达到 Spec 01 的 5 万–20 万连接 / 1 千–1 万 msg/s。

单机压测源自同一 IP，最先撞到的是 Gateway 接入限流（每 IP 5 conn/s），因此本机吞吐**不能外推**为容量结论。实测数字与边界见 [`local-measured-baseline.md`](../load_test/baselines/local-measured-baseline.md)。
...
| 故障演练 | `docs/chaos/01`–`06` + `livechat-server/scripts/chaos/`（01、02 有实测复盘） | 降级路径 |
| 实测基线 | `load_test/baselines/local-measured-baseline.md` | 五场景数字 + 天花板在哪 |
...
| **理解「为什么这样测」、对外怎么讲** | [`验证链路设计方法论.md`](./验证链路设计方法论.md) |
| 跑压测 / 模拟高负载 | `docs/load-practice/` · `load_test/` |
| 演练故障 | `docs/chaos/` |
...
- 路由注册失败时服务端**无主动降级信号**（客户端需靠超时/补拉发现静默降级）  
- E2EE、音视频、全球分片等见 Spec 非目标  

已补齐（勿再当缺口）：发送侧 Outbox 背压 429（0032）、protobuf 真握手与实时投递验证（0034）。
