...
- [ ] `load_test/scenarios/connect.py`：真 WebSocket upgrade（protobuf 握手按最小可用补齐）
- [ ] `load_test/scenarios/sync_backfill.py`：造 cursor 落后 + 拉取 `/v1/sync/events`，输出延迟/吞吐/错误率
- [ ] `load_test/scenarios/send_message.py`：去掉脆弱 `psql` 依赖；用已登录用户 + 建群或 1:1 API（若 [0026](0026-server-direct-conversation-api.md) 未就绪则用群会话 workaround，并在 README 注明）
- [ ] `group_fanout`：支持可配置成员数；默认可抬到能触发热点阈值的量级；能对照 `ErrGroupBusy` / hot-group 日志
- [ ] 跑一轮本机基线，把 P50/P95/错误率写入 `load_test/baselines/`（替换纯差距说明）
- [ ] chaos 04：补最小 push 延迟/失败注入（env 开关或 mock provider sleep），与 [docs/chaos/04-push-delay.md](../docs/chaos/04-push-delay.md) 对齐
- [ ] README / 手册命令与实际参数一致
