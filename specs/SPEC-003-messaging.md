...
CREATE TABLE inbox (
  ...
  -- 幂等键: 扇出重试（006）分配到新 inbox_seq 时, 主键挡不住重复,
  -- 靠这条唯一约束保证同一条消息对同一用户恰好投递一次
...
**收件箱保留策略（与 SPEC-010 多设备协同定义）**：inbox 是投递队列不是
存储——条目生命周期 = `max(30 天, 该用户全部设备的 max(last_active_at) + 7 天)`，
到期分区物理删除。某设备（如吃灰的 iPad）的游标落后于保留窗口时，
SyncResponse 返回 `gap_detected=true`，客户端降级为按会话 `conv_seq` 范围
全量拉取（messages 表永久保留，数据不丢，只是同步路径变贵）。
这回答了 per-device 游标（010）下"最慢设备拖着 inbox 无法回收"的问题：
不等最慢设备，窗口外直接删，靠降级路径兜底。
...
3. 服务端返回 `(since, since+N]` 的收件箱条目（分页，每页 200）；
   若 `since` 已落在保留窗口之外，返回 `gap_detected=true`，客户端转
   per-conversation 范围拉取（见上文保留策略）；
...
**AckDelivered 延迟预算（SPEC-008 兜底窗口的推导依据）**：在基准负载
（1,000 msg/s）下，在线设备从消息落库到服务端收到其 `AckDelivered` 的
链路预算为 **p99 < 2s、p99.9 < 7s**（推送 RPC + 客户端落库 + 上行回执）。
该指标进 SPEC-005 面板持续监测；SPEC-008 的 APNs 兜底窗口 = p99.9 + 3s
margin = 10s，预算变化时兜底窗口随之调整。
...
- 收件箱条目按上文保留策略到期删除（分区 drop），历史消息永久保留在 messages。
