...
- **10s 不是拍脑袋（评审 C4）**：SPEC-003 已定义在线设备 AckDelivered
  延迟预算 p99.9 < 7s，兜底窗口 = 7s + 3s margin = 10s，即正常情况下
  最多 0.1% 的在线消息会触发多余的 APNs 兜底。若 SPEC-005 面板显示
  AckDelivered p99.9 漂移超预算，先修链路拥塞，其次才是放宽窗口
  ——否则推送量翻倍会掩盖真正的性能退化。
