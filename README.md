...
**仓库现状 vs 商用 IM 差距**：[`docs/仓库现状与商用品差距.md`](docs/仓库现状与商用品差距.md)。  
**Go 基础偏弱、对照本仓技术栈复盘**：[`docs/Go后端技术栈与学习导读.md`](docs/Go后端技术栈与学习导读.md)。  
...
**iOS 复习导读 / 收发边界（0035–0041）**：[`docs/ios-client-study-guide.md`](docs/ios-client-study-guide.md)。  
**iOS 0041 实时测试设计**：[`docs/ios-0041-realtime-test-design.md`](docs/ios-0041-realtime-test-design.md)。  
**iOS App 本机联调 / 手机号格式**：[`docs/ios-app-testing.md`](docs/ios-app-testing.md)。
...
├── docs/
│   ├── 架构设计总览.md         # ← 新人 / 复盘首选
│   ├── Go后端技术栈与学习导读.md # Go 弱基础：技术栈 ↔ 代码
│   ├── 高负载IM验证计划.md     # 高负载验证阶段与 0029–0034
│   ├── 验证链路设计方法论.md   # 压测/混沌：原则、场景设计、对外叙事
│   ├── adr/                    # 架构决策记录
│   ├── chaos/                  # 故障演练手册
...
- Phase 1–3 学习闭环已在 `livechat-server/`、`load_test/`、`docs/chaos/` 落地；iOS 重写与交互票见 `issues/`（至 0064 无开放票）
- 规格以 `Specs/` 为唯一设计源；领域术语见 `CONTEXT.md`
- 架构快照入口：`docs/架构设计总览.md`
- 现状与商用差距：`docs/仓库现状与商用品差距.md`
